...
// display random review box
    $rand_review_query = tep_db_query("select substring(reviews_text, 1, 400) as reviews_text from " . TABLE_REVIEWS_DESCRIPTION . " where reviews_id = '" . (int)$random_product['reviews_id'] . "' and languages_id = '" . (int)$languages_id . "'");
    ...
    $rand_review_text = tep_break_string(tep_output_string_protected($rand_review['reviews_text']), 15, '-<br>');
	 ...
	 $name_prod = '<span><a href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS_INFO, 'products_id=' . $random_product['products_id'] . '&reviews_id=' . $random_product['reviews_id']) . '">' . $random_product['products_name']. '</a></span>';
    ...
    $info_box_contents[] = array('text' => '
                    <table cellpadding="0" cellspacing="0" border="0">
						...
						<tr><td class="desc_padd">'.$review_prod.'</td></tr>
						<tr><td class="star_padd">
' . tep_image(DIR_WS_IMAGES . 'stars_' . $random_product['reviews_rating'] . '.gif' , sprintf(BOX_REVIEWS_TEXT_OF_5_STARS, $random_product['reviews_rating'])) . '</td></tr>
                    </table>');
...
// display 'write a review' box
	...
	<table border="0" cellspacing="5" cellpadding="0">
		<tr><td><a href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, 'products_id=' . $HTTP_GET_VARS['products_id']) . '">' . tep_image(DIR_WS_IMAGES . 'box_write_review.gif', IMAGE_BUTTON_WRITE_REVIEW) . '</a></td>
