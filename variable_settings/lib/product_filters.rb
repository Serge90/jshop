...
#   Product.scope :model_any,
#       lambda {|*opts|
#         conds = opts.map {|o| ProductFilters.brand_filter[:conds][o]}.reject {|c| c.nil?}
#         Product.with_property("model").conditions_any(conds)
#       }

    def self.model_filter
      models = ProductProperty.find_all_by_property_id(@@model_property).map(&:value).compact.uniq
      conds  = Hash[*models.map {|b| [b, "product_properties.value = '#{b}'"]}.flatten]
      { :name   => "Models",
        :scope  => :model_any,
        :conds  => conds,
        :labels => (models.sort).map {|k| [k,k]}
      }
    end
  
  Product.scope :selective_model_any, lambda {opts|| Product.model_any(opts)}
  def ProductFilters.selective_model_filter(taxon = nil)
      if taxon.nil?
        taxon = Taxonomy.first.root
      end
      all_models = ProductProperty.find_all_by_property_id(@@model_property).map(&:value).uniq
      scope = ProductProperty.scoped(:conditions => ["property_id = ?", @@model_property]).
                              scoped(:joins      => {:product => :taxons},
                                     :conditions => ["taxons.id in (?)", [taxon] + taxon.descendants])
      models = scope.map {|p| p.value}

      { :name   => "Applicable Models",
        :scope  => :selective_model_any,
        :conds  => Hash[*all_models.map {|m| [m, "p_colour.value like '%#{m}%'"]}.flatten],
        :labels => models.sort.map {|k| [k,k]}
      }
    end
